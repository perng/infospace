...
- `examples/basel-problem/` — AI-generated: brief.md → talk.spec.yaml (Layer B) → document.json; `npm run spec:compile` after editing the spec, `generate:regen` preserves `locked: true` scenes; provenance sidecar.
...
- `packages/cli/src/journey-cli.ts` — validate / outline / resolve / spec-compile / diff / narration / manim CLI; takes a journey-module path, run from the presentation's root.
- `packages/cli/src/generate-journey.ts` + `packages/core/src/specCompiler.ts` — the M5 generator (registry-grounded prompt → Authoring Spec → document; engines: claude|codex CLI, env-scrubbed for nested runs) and the Layer B → authored-document expansion.
